defmodule C4diCoffeeWeb.SummaryController do
  ...
  def index(conn, _params) do
    pots = Repo.all(Pot)
    summaries = Enum.map(pots, fn p -> %Summary{pot_name: p.pot_name, avg_cup_size: 2, fill_level: 2, time: :calendar.local_time()} end)
      #ReadingsToSummary(Repo.all(from r in Reading, where: r.pot_name == p.pot_name),p) end)  
...
# def ReadingsToSummary(readings, pot_detail) do
